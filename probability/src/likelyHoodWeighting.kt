...
fun likelyHoodWeighting(x: Node, root: Node, dataSet: Set<Node>, n: Int): List<Double> {
    ...
    for (i in 1..n) {
        val w = weightedSample(root, dataSet, 1.0)
        xW[x.value] = xW[x.value]!! + w
    }
    return xW.values.toList().normalize()
}

private fun weightedSample(node: Node, e: Set<Node>, weight: Double): Double {
    var w = weight
    if (node !in e) node.randomValue()
    else w *= node.p()
    node.children.forEach {
        if (it.parents.size == 1 || it.parents.indexOf(node) == it.parents.size-1)
            w = weightedSample(it, e, w)
    }
    return w
}
